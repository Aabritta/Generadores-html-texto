    ...
    def format_with_passages(self):
        ...
        for word in verse.split(' '):
            if '@' not in word:
                ...
                variants = word.split('@')
                words.append("<span id=\"variant-" + variants[0] + "\"></span>")
                self.variants[variants[0]] = variants
        verse = ' '.join(words)
        self.formatted_body = verse
        ...
        for verse in self.verses:
            ...
            for note in notes:
                try:
                    ...
                    print("Error procesando nota en verso {n} {v}"
                        .format(n=verse.number, v=body,
                            p=note.passage), file=sys.stderr)
                    raise e
...
def generate_document(translation, greek, notes, canto, color):
    ...
    text = list(zip(translation.verses, greek.verses))
    name = 'canto' + canto + '.html'
    for verse in text:
        verse[0].format_with_passages()
        verse[1].format_with_passages()
...
def get_notes_greek(canto):
    ...
    for match in matches:
        try:
            ...
            print("Error procesando nota en canto {canto}, pasaje {pasaje}".format(canto=canto, pasaje=passage))
            raise e
    return notes
